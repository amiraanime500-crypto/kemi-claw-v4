...
async def add_scan_job(name, target, goal, chat_id, schedule="0 */6 * * *"):
    ...
    async def _run():
        try:
            ...
            await send_message(chat_id, f"➭➥ فدح دوري — {name}\\n🎥 `{target}`\\n📆 {len(results)} صضقه | {int(rate)}% قياح"try
        except AsyncioError as e: print(f"[Scheduler] {name}: {e}")
...
def list_jobs(): return [{"name":k, **v} for k,v in _jobs.items()]
def remove_job(name):
    try: scheduler.remove_job(f"kemi_{name}"); _jobs.pop(name,None); return {"ok":True}
