...
def _validate_plan(plan: dict) -> dict:
    ...
    return {"steps": [{"tool": s.get("tool", "unknown"), "args": s.get("args", {}), "rationale": s.get("rationale", ""), "step": s.get("step", i + 1)} for i, s in enumerate(steps) if isinstance(s, dict) and "tool" in s]}
