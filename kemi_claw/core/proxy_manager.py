...
if proxy_list:
    _proxies = [p.strip() for p in proxy_list.split(",") if p.strip()]
...
def get_next_proxy():
    ...
    p = _proxies[_current_idx % len(_proxies)]
    _current_idx += 1
    return p


async def rate_limited_request(domain: str):
    l = _rate_limits[domain]
    now = time.time()
    if now - l["window_start"] > 1.0:
        l["count"] = 0
        l["window_start"] = now
    if l["count"] >= DOMAIN_BURST:
        await asyncio.sleep(1.0 / DOMAIN_RATE)
    if l["count"] > 0:
        min_i = 1.0 / DOMAIN_RATE
        elapsed = now - l["last_request"]
        if elapsed < min_i:
            await asyncio.sleep(min_i - elapsed)
    l["last_request"] = time.time()
    l["count"] += 1
...
def get_rate_stats(domain: str = None):
    if domain:
        l = _rate_limits[domain]
        return {"domain": domain, "requests_this_second": l["count"]}
    return {"tracked_domains": len(_rate_limits), "proxies_available": len(_proxies)}
